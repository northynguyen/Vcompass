...
const LeftSideBar = ({ onFilterChange }) => {
    ...
    }, [priceRange, sortOrder, selectedAmenities, roomSize, rating]); // Chạy khi bất kỳ trạng thái nào thay đổi
    ...
    const handlePriceChange = (values) => {
        ...
        setPriceRange({
            min: Math.max(0, Math.min(70000000, newMin)),
            max: Math.max(0, Math.min(70000000, newMax)),
        });
    ...
    const handleInputChange = (e) => {
        ...
        const numericValue = Math.max(0, Math.min(70000000, Number(value)));
        setPriceRange((prev) => ({
            ...prev,
            [name]: numericValue,
        }));
